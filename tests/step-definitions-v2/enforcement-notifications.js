...
When('I search for a partnership between {string} and {string}', function (authority, organisation) {
    ...
    return shared
        ...
        .assert.containsText('h1.govuk-heading-l', 'Search for a partnership')
        .setValue('#edit-keywords', organisation)
        .click('#edit-submit-partnership-search')
        .clickLinkByPureText(partnership)
        .assert.containsText('h1.govuk-heading-l', organisation)
...
When('I raise a new enforcement against a partnership', function () {
    return shared
        ...
        .assert.containsText('h1.govuk-heading-l','Have you discussed this issue with the Primary Authority?')
        .click('#edit-next')
        .assert.containsText('h1.govuk-heading-l','Contact details for enforcement officer')
        .click('#edit-next')
});
When('I choose a coordinated member to enforce', function () {
    return shared
        .click('input[name="par_data_organisation_id"]:nth-child(1)')
        .assert.containsText('h1.govuk-heading-l','Enforce member')
        .click('#edit-next')
});
When('I choose an existing legal entity to enforce', function () {
    return shared
        .click('input[name="legal_entities_select"]:nth-child(1)')
        .assert.containsText('h1.govuk-heading-l','Enforce legal entity')
        .click('#edit-next')
});
...
When('I enter the details of a proposed enforcement', function () {
    return shared
        ...
        .assert.containsText('h1.govuk-heading-l','Enforcement details')
        .click('#edit-next')
});
When('I add an enforcement action {string}', function (title) {
    return shared
        ...
        .assert.containsText('h1.govuk-heading-l','Add an action to the enforcement notice')
        .click('#edit-next')
});
...
When('I review the enforcement notice', function () {
    return shared
        .assert.containsText('h1.govuk-heading-l','Review the enforcement notice')
        .click('#edit-save')
        .assert.containsText('h1.govuk-heading-l','Enforcement notice sent')
        .click('.button')
});
...
When('I search for an enforcement notice {string} against {string}', function (enforcement_notice, organisation) {
    return shared
        ...
        .assert.containsText('h1.govuk-heading-l', 'Enforcements')
...
When('I approve a single enforcement notice', function () {
    return shared
        .assert.containsText('h1.govuk-heading-l', 'Respond to notice of enforcement action')
...
When('I refer a single enforcement notice to {string}', function (authority) {
    return shared
        ...
        .click('#edit-next')
        .assert.containsText('h1.govuk-heading-l', 'Refer Enforcement Notice')
...
When('I review an enforcement notice submitted by {string}', function (enforcing_officer) {
    return shared
        .assert.containsText('h1.govuk-heading-l', 'Respond to notice of enforcement action')
        .assert.containsText('h1.govuk-heading-l', 'Review')
